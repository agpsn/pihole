...
#put contents of each url into a file, excluding any line starting with # or empty line
	...
	for URL in $(cat urls-sorted.txt | grep -v ^"#" | grep -v "^$"); do wget -O- -q $URL >> blocklist2a.txt; done && rm -f urls-sorted.txt

#remove duplicates from output
	echo "removing duplicates from output"
	cat blocklist2a.txt | sort -u > blocklist2.txt && rm blocklist2a.txt
...
#github updating
	...
	git lfs track blocklist2.txt; git add . && git commit -m "Updated" && git push
