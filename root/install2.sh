...
sed -i "/^lighttpd /i cp -n \/stubby.yml \/config/" /etc/s6-overlay/s6-rc.d/lighttpd/run
sed -i "/^lighttpd /i cp -n \/cloudflared.yml \/config/" /etc/s6-overlay/s6-rc.d/lighttpd/run
